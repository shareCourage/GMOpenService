...
/** Represents the results of executing a query on an `<GFMDatabase>`.
...
@interface GFMResultSet : NSObject {
    GFMDatabase          *_parentDB;
    GFMStatement         *_statement;
...
/** `GFMStatement` used by result set. */

@property (atomic, retain) GFMStatement *statement;

///------------------------------------
/// @name Creating and closing database
///------------------------------------

/** Create result set from `<GFMStatement>`
 
 @param statement A `<GFMStatement>` to be performed
 
 @param aDB A `<GFMDatabase>` to be used
 
 @return A `GFMResultSet` on success; `nil` on failure
 */

+ (instancetype)resultSetWithStatement:(GFMStatement *)statement usingParentDatabase:(GFMDatabase*)aDB;

/** Close result set */

- (void)close;

- (void)setParentDB:(GFMDatabase *)newDb;
