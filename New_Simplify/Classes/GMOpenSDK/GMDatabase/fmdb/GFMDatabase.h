...
#import "GFMDatabasePool.h"
    ...
    #define GFMDBAutorelease(__v) ([__v autorelease]);
    #define GFMDBReturnAutoreleased GFMDBAutorelease

    #define GFMDBRetain(__v) ([__v retain]);
    #define GFMDBReturnRetained GFMDBRetain

    #define GFMDBRelease(__v) ([__v release]);

    #define GFMDBDispatchQueueRelease(__v) (dispatch_release(__v));
#else
    // -fobjc-arc
    #define GFMDBAutorelease(__v)
    #define GFMDBReturnAutoreleased(__v) (__v)

    #define GFMDBRetain(__v)
    #define GFMDBReturnRetained(__v) (__v)

    #define GFMDBRelease(__v)

// If OS_OBJECT_USE_OBJC=1, then the dispatch objects will be treated like ObjC objects
// and will participate in ARC.
// See the section on "Dispatch Queues and Automatic Reference Counting" in "Grand Central Dispatch (GCD) Reference" for details. 
    #if OS_OBJECT_USE_OBJC
        #define GFMDBDispatchQueueRelease(__v)
    #else
        #define GFMDBDispatchQueueRelease(__v) (dispatch_release(__v));
    #endif
#endif
...
typedef int(^GFMDBExecuteStatementsCallbackBlock)(NSDictionary *resultsDictionary);


/** A SQLite ([http://sqlite.org/](http://sqlite.org/)) Objective-C wrapper.
 
 ### Usage
 The three main classes in GFMDB are:

 - `GFMDatabase` - Represents a single SQLite database.  Used for executing SQL statements.
 - `<GFMResultSet>` - Represents the results of executing a query on an `GFMDatabase`.
 - `<GFMDatabaseQueue>` - If you want to perform queries and updates on multiple threads, you'll want to use this class.

 ### See also
 
 - `<GFMDatabasePool>` - A pool of `GFMDatabase` objects.
 - `<GFMStatement>` - A wrapper for `sqlite_stmt`.
 
 ### External links
 
 - [GFMDB on GitHub](https://github.com/ccgus/GFMdb) including introductory documentation
 - [SQLite web site](http://sqlite.org/)
 - [GFMDB mailing list](http://groups.google.com/group/GFMdb)
 - [SQLite FAQ](http://www.sqlite.org/faq.html)
 
 @warning Do not instantiate a single `GFMDatabase` object and use it across multiple threads. Instead, use `<GFMDatabaseQueue>`.
...
/** Create a `GFMDatabase` object.
 
 An `GFMDatabase` is created with a path to a SQLite database file.  This path can be one of these three:

 1. A file system path.  The file does not have to exist on disk.  If it does not exist, it is created for you.
 2. An empty string (`@""`).  An empty database is created at a temporary location.  This database is deleted with the `GFMDatabase` connection is closed.
 3. `nil`.  An in-memory database is created.  This database will be destroyed with the `GFMDatabase` connection is closed.

 For example, to create/open a database in your Mac OS X `tmp` folder:

    GFMDatabase *db = [GFMDatabase databaseWithPath:@"/tmp/tmp.db"];

 Or, in iOS, you might open a database in the app's `Documents` directory:

    NSString *docsPath = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES)[0];
    NSString *dbPath   = [docsPath stringByAppendingPathComponent:@"test.db"];
    GFMDatabase *db     = [GFMDatabase databaseWithPath:dbPath];
 ...
 @return `GFMDatabase` object if successful; `nil` if failure.

 */

+ (instancetype)databaseWithPath:(NSString*)inPath;

/** Initialize a `GFMDatabase` object.
...
/** Execute single update statement
 ...
 @note If you want to use this from Swift, please note that you must include `GFMDatabaseVariadic.swift` in your project. Without that, you cannot use this method directly, and instead have to use methods such as `<executeUpdate:withArgumentsInArray:>`.
 */
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(GFMDBExecuteStatementsCallbackBlock)block;
...
/** Execute select statement
 ...
 Executing queries returns an `<GFMResultSet>` object if successful, and `nil` upon failure.  Like executing updates, there is a variant that accepts an `NSError **` parameter.  Otherwise you should use the `<lastErrorMessage>` and `<lastErrorMessage>` methods to determine why a query failed.
 
 In order to iterate through the results of your query, you use a `while()` loop.  You also need to "step" (via `<[GFMResultSet next]>`) from one record to the other.
 ...
 @return A `<GFMResultSet>` for the result set upon success; `nil` upon failure. If failed, you can call `<lastError>`, `<lastErrorCode>`, or `<lastErrorMessage>` for diagnostic information regarding the failure.
 
 @see GFMResultSet
 @see [`GFMResultSet next`](<[GFMResultSet next]>)
 @see [`sqlite3_bind`](http://sqlite.org/c3ref/bind_blob.html)
 */

- (GFMResultSet *)executeQuery:(NSString*)sql, ...;
...
/** Execute select statement
 ...
 @note If you want to use this from Swift, please note that you must include `GFMDatabaseVariadic.swift` in your project. Without that, you cannot use this method directly, and instead have to use methods such as `<executeQuery:withArgumentsInArray:>`.

 */

- (GFMResultSet *)executeQueryWithFormat:(NSString*)format, ... NS_FORMAT_FUNCTION(1,2);
...
- (GFMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments;
...
- (GFMResultSet *)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary *)arguments;


// Documentation forthcoming.
- (GFMResultSet *)executeQuery:(NSString*)sql withVAList: (va_list)args;
...
/** Adds SQL functions or aggregates or to redefine the behavior of existing SQL functions or aggregates.
 ...
 For example:
    ...
    [queue inDatabase:^(GFMDatabase *adb) {
        ...
        int rowCount = 0;
        GFMResultSet *ars = [adb executeQuery:@"select * from ftest where StringStartsWithH(foo)"];
        while ([ars next]) {
            rowCount++;
            NSLog(@"Does %@ start with 'h'?", [rs stringForColumnIndex:0]);
        }
        GFMDBQuickCheck(rowCount == 2);
    }];
...
/** Generate an `NSDateFormatter` that won't be broken by permutations of timezones or locales.
 ...
 Example:
    ...
    myDB.dateFormat = [GFMDatabase storeableDateFormat:@"yyyy-MM-dd HH:mm:ss"];
 ...
 @warning Note that `NSDateFormatter` is not thread-safe, so the formatter generated by this method should be assigned to only one GFMDB instance and should not be used for other purposes.
...
/** Set to a date formatter to use string dates with sqlite instead of the default UNIX timestamps.
 ...
 @param format Set to nil to use UNIX timestamps. Defaults to nil. Should be set using a formatter generated using GFMDatabase::storeableDateFormat.
 ...
 @warning Note there is no direct getter for the `NSDateFormatter`, and you should not use the formatter you pass to GFMDB for other purposes, as `NSDateFormatter` is not thread-safe.
 */
...
/** Objective-C wrapper for `sqlite3_stmt`
 ...
 This is a wrapper for a SQLite `sqlite3_stmt`. Generally when using GFMDB you will not need to interact directly with `GFMStatement`, but rather with `<GFMDatabase>` and `<GFMResultSet>` only.
...
@interface GFMStatement : NSObject {
