...
+ (instancetype)resultSetWithStatement:(GFMStatement *)statement usingParentDatabase:(GFMDatabase*)aDB {
    ...
    return GFMDBReturnAutoreleased(rs);
}
...
- (void)dealloc {
    ...
    GFMDBRelease(_columnNameToIndexMap);
...
- (NSDictionary*)resultDict {
    ...
    if (num_cols > 0) {
        ...
        return GFMDBReturnAutoreleased([dict copy]);
    }
    else {
    ...
    if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
        ...
        if (outErr) {
            if (_parentDB) {
                ...
                *outErr = [NSError errorWithDomain:@"GFMDatabase" code:SQLITE_MISUSE userInfo:errorMessage];
            }
