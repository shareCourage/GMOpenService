...
- (GFMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args;
...
#pragma mark GFMDatabase instantiation and deallocation

+ (instancetype)databaseWithPath:(NSString*)aPath {
    return GFMDBReturnAutoreleased([[self alloc] initWithPath:aPath]);
}
...
- (void)dealloc {
    [self close];
    GFMDBRelease(_openResultSets);
    GFMDBRelease(_cachedStatements);
    GFMDBRelease(_dateFormat);
    GFMDBRelease(_databasePath);
    GFMDBRelease(_openFunctions);
...
// /* need to make sure to do X with GFMDB version 2.4 or later */
// if ([GFMDatabase GFMDBVersion] >= 0x0240) { … }

+ (SInt32)GFMDBVersion {
    
    // we go through these hoops so that we only have to change the version number in a single spot.
    static dispatch_once_t once;
    static SInt32 GFMDBVersionVal = 0;
    
    dispatch_once(&once, ^{
        NSString *prodVersion = [self GFMDBUserVersion];
        ...
        char *e = nil;
        GFMDBVersionVal = (int) strtoul([junk UTF8String], &e, 16);
        
    });
    

    return GFMDBVersionVal;
}
...
static int GFMDBDatabaseBusyHandler(void *f, int count) {
    GFMDatabase *self = (__bridge GFMDatabase*)f;
...
- (void)setMaxBusyRetryTimeInterval:(NSTimeInterval)timeout {
    ...
    if (timeout > 0) {
        sqlite3_busy_handler(_db, &GFMDBDatabaseBusyHandler, (__bridge void *)(self));
    }
    else {
...
// but for folks who don't bother noticing that the interface to GFMDatabase changed,
// we'll still implement the method so they don't get suprise crashes
- (int)busyRetryTimeout {
    NSLog(@"%s:%d", __FUNCTION__, __LINE__);
    NSLog(@"GFMDB: busyRetryTimeout no longer works, please use maxBusyRetryTimeInterval");
    return -1;
}

- (void)setBusyRetryTimeout:(int)i {
    NSLog(@"%s:%d", __FUNCTION__, __LINE__);
    NSLog(@"GFMDB: setBusyRetryTimeout does nothing, please use setMaxBusyRetryTimeInterval:");
}
...
- (void)closeOpenResultSets {
    ...
    NSSet *openSetCopy = GFMDBReturnAutoreleased([_openResultSets copy]);
    for (NSValue *rsInWrappedInATastyValueMeal in openSetCopy) {
        GFMResultSet *rs = (GFMResultSet *)[rsInWrappedInATastyValueMeal pointerValue];
...
- (void)resultSetDidClose:(GFMResultSet *)resultSet {
...
- (GFMStatement*)cachedStatementForQuery:(NSString*)query {
    
    NSMutableSet* statements = [_cachedStatements objectForKey:query];
    
    return [[statements objectsPassingTest:^BOOL(GFMStatement* statement, BOOL *stop) {
...
- (void)setCachedStatement:(GFMStatement*)statement forQuery:(NSString*)query {
...
+ (NSDateFormatter *)storeableDateFormat:(NSString *)format {
    ...
    NSDateFormatter *result = GFMDBReturnAutoreleased([[NSDateFormatter alloc] init]);
    result.dateFormat = format;
    result.timeZone = [NSTimeZone timeZoneForSecondsFromGMT:0];
    result.locale = GFMDBReturnAutoreleased([[NSLocale alloc] initWithLocaleIdentifier:@"en_US"]);
    return result;
}
...
- (void)setDateFormat:(NSDateFormatter *)format {
    GFMDBAutorelease(_dateFormat);
    _dateFormat = GFMDBReturnRetained(format);
}
...
- (BOOL)goodConnection {
    ...
    GFMResultSet *rs = [self executeQuery:@"select name from sqlite_master where type='table'"];
...
- (void)warnInUse {
    NSLog(@"The GFMDatabase %@ is currently in use.", self);
    
#ifndef NS_BLOCK_ASSERTIONS
    if (_crashOnErrors) {
        NSAssert(false, @"The GFMDatabase %@ is currently in use.", self);
        abort();
    }
#endif
}
...
- (BOOL)databaseExists {
    ...
    if (!_db) {
        ...
        NSLog(@"The GFMDatabase %@ is not open.", self);
        
    #ifndef NS_BLOCK_ASSERTIONS
        if (_crashOnErrors) {
            NSAssert(false, @"The GFMDatabase %@ is not open.", self);
            abort();
        }
    #endif
...
- (NSError*)errorWithMessage:(NSString*)message {
    ...
    return [NSError errorWithDomain:@"GFMDatabase" code:sqlite3_errcode(_db) userInfo:errorMessage];    
}
...
- (GFMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    GFMStatement *statement  = 0x00;
    GFMResultSet *rs         = 0x00;
    ...
    GFMDBRetain(statement); // to balance the release below
    
    if (!statement) {
        statement = [[GFMStatement alloc] init];
    ...
    rs = [GFMResultSet resultSetWithStatement:statement usingParentDatabase:self];
    [rs setQuery:sql];
...
- (GFMResultSet *)executeQueryWithFormat:(NSString*)format, ... {
    va_list args;
...
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    if (_shouldCacheStatements && !cachedStmt) {
        cachedStmt = [[GFMStatement alloc] init];
        
        [cachedStmt setStatement:pStmt];
        
        [self setCachedStatement:cachedStmt forQuery:sql];
        
        GFMDBRelease(cachedStmt);
    }
...
int GFMDBExecuteBulkSQLCallback(void *theBlockAsVoid, int columns, char **values, char **names); // shhh clang.
int GFMDBExecuteBulkSQLCallback(void *theBlockAsVoid, int columns, char **values, char **names) {
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(GFMDBExecuteStatementsCallbackBlock)block {
    ...
    rc = sqlite3_exec([self sqliteHandle], [sql UTF8String], block ? GFMDBExecuteBulkSQLCallback : nil, (__bridge void *)(block), &errmsg);
...
static NSString *GFMDBEscapeSavePointName(NSString *savepointName) {
...
- (BOOL)startSavePointWithName:(NSString*)name error:(NSError**)outErr {
    ...
    NSString *sql = [NSString stringWithFormat:@"savepoint '%@';", GFMDBEscapeSavePointName(name)];
...
- (BOOL)releaseSavePointWithName:(NSString*)name error:(NSError**)outErr {
    ...
    NSString *sql = [NSString stringWithFormat:@"release savepoint '%@';", GFMDBEscapeSavePointName(name)];
...
- (BOOL)rollbackToSavePointWithName:(NSString*)name error:(NSError**)outErr {
    ...
    NSString *sql = [NSString stringWithFormat:@"rollback transaction to savepoint '%@';", GFMDBEscapeSavePointName(name)];
    BOOL worked = [self executeUpdate:sql];
...
void GFMDBBlockSQLiteCallBackFunction(sqlite3_context *context, int argc, sqlite3_value **argv); // -Wmissing-prototypes
void GFMDBBlockSQLiteCallBackFunction(sqlite3_context *context, int argc, sqlite3_value **argv) {
#if ! __has_feature(objc_arc)
...
- (void)makeFunctionNamed:(NSString*)name maximumArguments:(int)count withBlock:(void (^)(sqlite3_context *context, int argc, sqlite3_value **argv))block {
    ...
    id b = GFMDBReturnAutoreleased([block copy]);
    
    [_openFunctions addObject:b];
    
    /* I tried adding custom functions to release the block when the connection is destroyed- but they seemed to never be called, so we use _openFunctions to store the values instead. */
#if ! __has_feature(objc_arc)
    sqlite3_create_function([self sqliteHandle], [name UTF8String], count, SQLITE_UTF8, (void*)b, &GFMDBBlockSQLiteCallBackFunction, 0x00, 0x00);
#else
    sqlite3_create_function([self sqliteHandle], [name UTF8String], count, SQLITE_UTF8, (__bridge void*)b, &GFMDBBlockSQLiteCallBackFunction, 0x00, 0x00);
#endif
}
