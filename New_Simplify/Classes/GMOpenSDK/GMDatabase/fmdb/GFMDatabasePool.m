...
@interface GFMDatabasePool()

- (void)pushDatabaseBackInPool:(GFMDatabase*)db;
- (GFMDatabase*)db;

@end


@implementation GFMDatabasePool
...
+ (instancetype)databasePoolWithPath:(NSString*)aPath flags:(int)openFlags {
    return GFMDBReturnAutoreleased([[self alloc] initWithPath:aPath flags:openFlags]);
}
...
- (instancetype)initWithPath:(NSString*)aPath flags:(int)openFlags {
    ...
    if (self != nil) {
        _path               = [aPath copy];
        _lockQueue          = dispatch_queue_create([[NSString stringWithFormat:@"GFMdb.%@", self] UTF8String], NULL);
        _databaseInPool     = GFMDBReturnRetained([NSMutableArray array]);
        _databaseOutPool    = GFMDBReturnRetained([NSMutableArray array]);
...
- (void)dealloc {
    ...
    _delegate = 0x00;
    GFMDBRelease(_path);
    GFMDBRelease(_databaseInPool);
    GFMDBRelease(_databaseOutPool);
    
    if (_lockQueue) {
        GFMDBDispatchQueueRelease(_lockQueue);
        _lockQueue = 0x00;
    }
...
- (void)pushDatabaseBackInPool:(GFMDatabase*)db {
    ...
    [self executeLocked:^() {
        ...
        if ([self->_databaseInPool containsObject:db]) {
            [[NSException exceptionWithName:@"Database already in pool" reason:@"The GFMDatabase being put back into the pool is already present in the pool" userInfo:nil] raise];
        }
...
- (GFMDatabase*)db {
    ...
    [self executeLocked:^() {
        ...
        if (db) {
            ...
            db = [GFMDatabase databaseWithPath:self->_path];
